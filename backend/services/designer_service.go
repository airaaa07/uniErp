...
func (s *DesignerService) GetRecord(recordID string) (*models.Record, error) {
	var record models.Record
...
func (s *DesignerService) UpdateRecord(recordID string, req models.RecordUpdate) (*models.Record, error) {
	dataJSON, err := json.Marshal(req.Data)
	if err != nil {
		return nil, err
	}
	...
	_, err = s.db.Exec(
		"UPDATE records SET data = $1, updated_at = NOW() WHERE record_id = $2",
...
func (s *DesignerService) DeleteRecord(recordID string) error {
	_, err := s.db.Exec("DELETE FROM records WHERE record_id = $1", recordID)
	return err
}

func (s *DesignerService) SearchRecords(moduleKey string, searchTerm string) ([]models.Record, error) {
	...
	if searchTerm != "" {
		...
		err = s.db.Select(&records,
			"SELECT * FROM records WHERE module_key = $1 AND data::text ILIKE $2 ORDER BY created_at DESC",
